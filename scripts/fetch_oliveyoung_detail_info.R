...
parse_args <- function(args) {
  out <- list(
    goods_no = NULL,
    display_category_number = "",
    tracking_code = "",
    output_dir = "data/raw/oliveyoung/detail",
  ...
  while (i <= length(args)) {
    ...
    if (identical(key, "--display-category-number")) out$display_category_number <- value
    if (identical(key, "--tracking-code")) out$tracking_code <- value
...
main <- function() {
  ...
  response <- httr2::request("https://www.oliveyoung.co.kr/goods/api/v1/detail") |>
    ...
    httr2::req_url_query(
      goodsNo = args$goods_no,
      dispCatNo = args$display_category_number,
      trackingCd = args$tracking_code
    ) |>
    httr2::req_perform()
  ...
  message("Saved detail response: ", output_path)
}
